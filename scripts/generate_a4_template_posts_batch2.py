...
ENGLISH_WORDS = ['cow', 'pig', 'fish', 'duck', 'cat', 'dog', 'hat', 'apple', 'balloon', 'grapes']
COUNT_OBJECTS = [('사과', 'apple'), ('풍선', 'balloon'), ('물고기', 'fish'), ('꽃', 'flower'), ('고양이', 'cat'), ('오리', 'duck'), ('조개', 'shell'), ('토끼', 'rabbit'), ('별', 'star'), ('포도', 'grapes')]
...
def counting_page(index):
    ...
    base.header(d, A4P, f'{count} 과일 숫자 학습지', 'scene · trace · more or less')
...
def tracing_page(index):
    img, d = base.page(A4P)
    base.header(d, A4P, '곡선 선긋기 학습지', 'curves · loops · picture finish')
...
def maze_page(index):
    ...
    base.header(d, A4P, '고난도 모험 미로', 'harder routes · more dead ends')
    return img
...
def cut_paste_page(index):
    ...
    d = ImageDraw.Draw(img)
    d.rectangle((40, 36, A4P[0]-40, 166), fill='white')
    base.header(d, A4P, '오려붙이기 종이 인형', 'puppet · dress-up · build')
    return img
...
def hidden_page(index):
    ...
    base.header(d, A4L, '교실 숨은 캐릭터 찾기', 'dense classroom search')
    return img
...
def main():
    sets = [
        ('count-fruit-a4', counting_page, POST_ROOT / '2026-06-26-count-apple.md', '과일 숫자 학습지', '세기, trace, 비교, draw-more를 묶은 숫자 학습지 세트.', [f'과일 숫자 {i}' for i in range(1, 11)], A4P),
        ('english-basic-a4', english_page, POST_ROOT / '2026-06-07-english-animal.md', '기초 영어 단어 학습지', '큰 그림, tracing, matching, sentence를 묶은 영어 학습지 세트.', [f'영어 단어 {i}' for i in range(1, 11)], A4P),
        ('tracing-curves-a4', tracing_page, POST_ROOT / '2026-06-16-curve-tracing.md', '곡선 선긋기 학습지', 'curve, loop, finish-the-picture를 묶은 선긋기 세트.', [f'곡선 선긋기 {i}' for i in range(1, 11)], A4P),
        ('maze-challenge-a4', maze_page, POST_ROOT / '2026-05-10-maze-rabbit.md', '고난도 모험 미로', '현재보다 더 복잡한 branch형 미로 세트.', [f'고난도 미로 {i}' for i in range(1, 11)], A4P),
        ('cut-paste-puppet-a4', cut_paste_page, POST_ROOT / '2026-05-29-cut-paste-farm.md', '오려붙이기 종이 인형', 'paper doll과 puppet 조립을 묶은 오려붙이기 세트.', [f'종이 인형 {i}' for i in range(1, 11)], A4P),
        ('hidden-wally-classroom-a4', hidden_page, POST_ROOT / '2026-04-22-hidden-classroom.md', '교실 숨은 캐릭터 찾기', 'target strip과 dense scene으로 만든 교실 탐색 세트.', [f'교실 탐색 {i}' for i in range(1, 11)], A4L),
    ]
