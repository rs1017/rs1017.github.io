...
| **image-generator** | 이미지 생성, 트렐로 카드 관리 | `.claude/agents/image-generator.md` |
...
**포스트 작성과 이미지 생성은 병렬로 진행됩니다:**

- 포스트 작성 시 이미지 생성을 **대기하지 않음**
- 이미지 링크만 먼저 포스트에 포함
- `image-generator` 에이전트에게 이미지 생성 위임
- 트렐로 카드로 진행 상황 추적
- 트렐로 완료 시 이미지 상태 업데이트

```yaml
이미지_프롬프트_저장_위치: _topics/drafting/{topic}.yml → media 섹션
이미지_품질_규칙:
  - 프롬프트에 항상 포함: "high quality, sharp focus, detailed, 4k, ultra detailed, crisp"
  - 흐릿함 방지: enhance=true 파라미터 사용
  - 해상도: 최소 1200x800
수동_생성_옵션:
  - Pollinations.ai (무료)
  - DALL-E 3 (유료)
...
| **trello** | 작업 관리/카드 생성 | Trello API | 무료 |

### Trello 연동 설정

```yaml
보드_ID: rs1017.github.io
용도: 블로그 이미지 생성 작업 관리
리스트_구조:
  - 이미지 생성 대기
  - 생성 중
